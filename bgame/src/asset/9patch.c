...
bgame_load_9patch(
	...
	return bgame_load_asset(bundle, "9patch", path, &config);
}
...
bgame_9patch_load(
	...
	bgame_9patch_config_t config = *(bgame_9patch_config_t*)args;

	if (
		!bgame_file_changed(bundle, path)
		&& (
	...
	if (result.code != CF_RESULT_SUCCESS) {
		log_error("Could not load image: %s", path);
